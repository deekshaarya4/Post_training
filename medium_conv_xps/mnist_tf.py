...
class MNISTInput(object):
    def __init__(self, config, name=None):
        ...
        self.epoch_size = self.data.test.num_examples // self.batch_size
...
class MNISTModel(object):
  ...
  def __init__(self, config,x_,y_,k_):
    ...
    softmax_w = weight_variable([size_last_layer,10 ])
    softmax_b = bias_variable( [10])
...
if __name__ == "__main__":
    ...
    if os.uname()[1]=="anguille" :
        print("copy files ?")
        if input():
            os.system('scp "%s" "%s:%s"' % ("mnist_tf.py", "lu", "/home/audiffren/python/deep/medium_conv_xps/mnist_tf.py" ) )
            print("files copied")
        
        print("get files ?")
        if input():
            try :
                os.system('scp "%s:%s" "%s" ' % ( "lu", "/home/audiffren/python/deep/medium_conv_xps/error_small_config", "error_small_config.txt" ) )
                os.system('scp "%s:%s" "%s" ' % ( "lu", "/home/audiffren/python/deep/medium_conv_xps/error_medium_config", "error_medium_config.txt" ) )
                os.system('scp "%s:%s" "%s" ' % ( "lu", "/home/audiffren/python/deep/medium_conv_xps/error_large_config", "error_large_config.txt" ) )
        ...
        if input():
            ...
            for fname in ["error_small_config","error_medium_config","error_large_config"]:
      ...
      if True :
          ...
          train_input = MNISTInput(config=config, name="TrainInput")
